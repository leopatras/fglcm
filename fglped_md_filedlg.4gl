...
PUBLIC TYPE FILEDLG_RECORD RECORD
  ...
  defaultpath STRING,
  opt_choose_directory
      SMALLINT, -- if this is set the user can only choose directories
...
FUNCTION filedlg_open(r)
  ...
  IF _isLocal() THEN
    CALL ui.Interface.frontCall(
        "standard",
        "openfile",
        [os.Path.pwd(), r.types[1].description, r.types[1].suffixes, r.title],
        [fn])
  ELSE
    ...
    IF r.opt_root_dir IS NOT NULL THEN
      IF NOT os.Path.exists(r.opt_root_dir)
          OR NOT os.Path.isDirectory(r.opt_root_dir) THEN
        CALL fgl_winMessage(
            "Error", SFMT("wrong root dir:%1", r.opt_root_dir), "error")
        RETURN NULL
      END IF
  ...
  DIALOG ATTRIBUTE(UNBUFFERED)
        ...
        IF _filedlg_mbox_yn(
          ...
          IF NOT file_delete(file_join(currpath, delfilename)) THEN
            CALL _filedlg_mbox_ok(
                "Error", SFMT("Can't delete %1", filename), "stop")
          END IF
    ...
    ON ACTION accept
      ...
      IF _filedlg_focusInTable("sr") THEN
        ...
        IF r.opt_choose_directory
            AND _filedlg_list[arr_curr()].entry == ".." THEN
      ...
      ELSE
        ...
        IF (file_on_windows() AND filename.getCharAt(2) == ":")
            OR (filename.getCharAt(1) = file_get_separator()) THEN
      ...
      IF r.opt_root_dir IS NOT NULL THEN --check the resulting file
        ...
        IF (os.Path.isFile(full) OR os.Path.isLink(full))
            OR (dlgtype == C_SAVE AND NOT os.Path.exists(full)) THEN
          LET full = os.Path.dirName(full)
        END IF
        IF NOT isInsideDirectoryParent(full, m_r.opt_root_dir) THEN
          LET errstr =
              SFMT(%"File or directory '%1' is outside your scope!", filename)
      ...
      IF NOT doContinue AND dlgtype = C_OPEN THEN
        IF NOT file_exists(filepath) THEN
          LET errstr =
              SFMT(%"File or directory '%1' does not exist!",
      ...
      IF NOT doContinue AND dlgtype = C_SAVE THEN
        ...
        IF file_exists(filepath) THEN
          IF NOT _filedlg_mbox_yn(
              "Warning",
              SFMT("File '%1' already exists, do you want to replace it ?",
                  filename),
              "question") THEN
            LET doContinue = 1
          END IF
...
FUNCTION _filedlg_getfiles_int(dirpath, typelist)
  ...
  WHILE TRUE
    ...
    END IF
    CALL _filedlg_appendEntry(
        isdir, fname, typelist, size, os.Path.mtime(pname), dirpath)
  END WHILE
  CALL os.Path.dirClose(dh)
  IF m_r.opt_choose_directory THEN
    MESSAGE SFMT("%1 %2 files in %3",
        _flat_files.getLength(), typelist, dirpath)
  END IF
...
FUNCTION _filedlg_appendEntry(isdir, name, typelist, size, modDate, dirpath)
  ...
  IF name = "." OR name.getIndexOf(".@", 1) <> 0 THEN
    RETURN
  ELSE
    IF NOT isdir AND NOT _filedlg_checktypeandext(typelist, name) THEN
      RETURN
    END IF
  END IF
